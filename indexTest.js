const { submitData } = require('../your-script-file.js');
...
describe("submitData()", () => {
  ...
  it("makes a POST request to /users with a name and email", async () => {
    ...
    chai.expect(headers['accept'][0]).to.equal('application/json');
    chai.expect(Object.keys(reqBody), "The request body should only have 'name' and 'email' key/value pairs").to.deep.equal(["name", "email"]);
    chai.expect(reqBody.name, "The 'name' property was not found in the request body").to.eq("Steve");
    chai.expect(reqBody.email, "The 'email' property was not found in the request body").to.eq("steve@steve.com");
  });
  ...
  it("handles the POST request response, retrieves the new id value and appends it to the DOM", async function () {
    ...
    chai.expect(document.body.innerHTML).to.include(rando);
  });
  ...
  it("handles a failed POST request using catch, appends the error message to the DOM", async function () {
    ...
    await submitData(name, email);
    chai.expect(document.body.innerHTML).to.include(message);
  });
});